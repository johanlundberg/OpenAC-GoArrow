...
For a destination in the current dungeon, the **Route** tab previews the client's
indoor path without starting a walk. It shows the planned walking distance and
ordered waypoints with cell IDs and landblock-local east, north, and height in
metres. The preview updates after the character moves or the target changes;
it reports when the host cannot find a path. It does not predict door actions
or portal transitions, and the waypoints are not drawn on schematic dungeon
images because those images have no world-coordinate calibration.
Outdoor route recalculation pauses inside dungeons and other indoor cells when no indoor leg can be resolved. For a named location in the current dungeon whose XML record includes an exact indoor cell position, **Go** with Auto-Navigate enabled asks OpenAC to path to that position through the indoor collision world. For a named destination such as **Sawato** or **Sawato Portal** inside Town Network, GoArrow can also match a live portal object named **Sawato Portal** or **Portal to Sawato** in the current indoor landblock and walk to it. A direct request to that name stops at the portal. An outdoor route that enters Town Network continues through its portal step automatically, walks to the named indoor portal, activates it, and resumes its remaining route after the next transition. A selected object in the same dungeon also works: use `/go selected`, then **Go**. OpenAC plans the indoor steps; GoArrow shows their waypoints in the Route tab but cannot place them on the schematic dungeon image.
When an outdoor destination is selected inside a dungeon, GoArrow checks nearby portal objects' destination labels to match the next route step, even if the portal's name differs. If the labels are missing and several Surface Portals are visible, it requests their appraisal and retries when the details arrive. A single nearby **Surface Portal** is the fallback exit. GoArrow walks to and activates the selected exit, then recalculates from the actual outdoor arrival point. If several exits remain indistinguishable, it waits for manual selection.
